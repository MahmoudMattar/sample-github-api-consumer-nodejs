//contains methods to work on our repository model
//our service layer. Its responsibility is to call the corresponding method of our repository model:
...
async function updateDatabase(responseData, owner, name) {
  ...
  if (responseData.repository.releases) {
    ...
    await RepositoryModel.findByOwnerAndName(owner, name).then(
      (oldGitHubRelease) => {
        ...
        console.log(
          `Updated latest release: http://github.com${repositoryData.resourcePath}`
        );
      }
    );
  }
...
async function getLatestRelease(repository) {
  ...
  await Axios.post(GITHUB_API_URL, jsonQuery, { headers: headers }).then(
    (response) => {
